<template>
  <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-white/[0.03]">
      <h5 class="mb-2 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">Profile Details</h5>
      <dl class="space-y-1.5">
        <div v-for="(item, i) in profileFields" :key="`pf-${i}`"
          class="flex items-center justify-between gap-3 border-b border-gray-100 pb-1.5 last:border-0 last:pb-0 dark:border-gray-800">
          <dt class="shrink-0 text-[11px] font-medium text-gray-400 dark:text-gray-500">{{ item.label }}</dt>
          <dd class="text-right text-xs font-medium text-gray-800 dark:text-gray-200">{{ item.value }}</dd>
        </div>
      </dl>
    </div>
    ...
    <!-- Contacts -->
    <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-white/[0.03]">
      <h5 class="mb-2 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">Contacts</h5>
      <div v-if="user.contacts?.length" class="space-y-1.5">
        <div
          v-for="contact in user.contacts"
          :key="`contact-${contact.id || contact.value}`"
          class="flex items-start justify-between gap-3 rounded-md border border-gray-100 bg-gray-50 px-2.5 py-2 dark:border-gray-800 dark:bg-gray-800/40"
        >
          <div>
            <p class="text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">{{ contact.type || 'contact' }}</p>
            <p class="mt-0.5 text-xs font-semibold text-gray-800 dark:text-gray-200">{{ contact.value || '-' }}</p>
          </div>
          <span
            v-if="contact.is_primary"
            class="mt-0.5 shrink-0 rounded-full bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium text-brand-600 dark:bg-brand-900/30 dark:text-brand-400"
          >Primary</span>
        </div>
      </div>
    ...
    <!-- Addresses -->
    <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-white/[0.03] lg:col-span-2">
      <h5 class="mb-2 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">Addresses</h5>
      <div v-if="user.addresses?.length" class="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div
          v-for="address in user.addresses"
          :key="`address-${address.id || address.address1}`"
          class="rounded-md border border-gray-100 bg-gray-50 px-2.5 py-2 dark:border-gray-800 dark:bg-gray-800/40"
        >
          ...
          <p class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ formatAddress(address) }}</p>
        </div>
      </div>
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-white/[0.03] lg:col-span-2">
      <div class="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h5 class="text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-gray-500">Office Shift Schedule</h5>
          <p class="mt-0.5 text-xs font-semibold text-gray-800 dark:text-gray-200">{{ officeShift?.name || 'No office shift assigned' }}</p>
        </div>
        <span
          v-if="officeShift?.grace_enabled"
          class="inline-flex w-fit rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-semibold text-violet-700 ring-1 ring-inset ring-violet-200 dark:bg-violet-900/20 dark:text-violet-300 dark:ring-violet-800"
        >
          Grace -{{ officeShift.grace_before_minutes || 0 }} / +{{ officeShift.grace_after_minutes || 0 }} min
        </span>
      </div>

      <div v-if="officeShiftSchedules.length" class="grid gap-2 sm:grid-cols-2">
        <div
          v-for="(schedule, index) in officeShiftSchedules"
          :key="`office-shift-schedule-${schedule.id || index}`"
          class="rounded-md border border-sky-100 bg-sky-50/70 px-2.5 py-2 dark:border-sky-900/50 dark:bg-sky-900/20"
        >
          ...
          <div class="grid grid-cols-2 gap-2">
            <div class="rounded-md bg-white px-2 py-1.5 dark:bg-gray-950/70">
              <p class="text-[10px] font-bold uppercase tracking-wide text-gray-400">Time In</p>
              <p class="mt-0.5 text-xs font-semibold text-gray-800 dark:text-gray-200">{{ formatScheduleTime(schedule.time_in) }}</p>
            </div>
            <div class="rounded-md bg-white px-2 py-1.5 dark:bg-gray-950/70">
              <p class="text-[10px] font-bold uppercase tracking-wide text-gray-400">Time Out</p>
              <p class="mt-0.5 text-xs font-semibold text-gray-800 dark:text-gray-200">{{ formatScheduleTime(schedule.time_out) }}</p>
            </div>
          </div>
        </div>
      </div>
