    ...
    <div v-if="open" :class="menuClass">
      <slot name="menu">
        ...
        <template v-for="(item, index) in menuItems">
          <p
            v-if="item.CHECKTYPE=='I'"
            :class="itemClass"
          >
          <Badge color="success">
            Check In {{ moment(item.CHECKTIME).format('LT') }}
          </Badge>
        ...
        />  --><Badge color="warning">
            Check Out {{ moment(item.CHECKTIME).format('LT') }}
          </Badge>
        </p>
        </template>
      </slot>
...
const props = defineProps({
  ...
  menuClass: {
    type: String,
    default:
      'absolute right-0 z-40 w-[180px] p-1 space-y-0 bg-white border border-gray-200 top-full rounded-2xl shadow-lg dark:border-gray-800 dark:bg-gray-dark',
  },
  itemClass: {
    type: String,
    default:
